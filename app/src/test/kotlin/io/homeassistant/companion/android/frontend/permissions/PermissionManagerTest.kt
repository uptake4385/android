...
class PermissionManagerTest {
    ...
    inner class CheckImprovPermissions {
        ...
        fun `Given rationale below cap when called then enqueues Improv with showRationale true`() = runTest {
            ...
            val pending = manager.pendingPermissionRequest.value
            assertInstanceOf(PermissionRequest.Improv::class.java, pending)
            val improv = pending as PermissionRequest.Improv
        ...
        fun `Given rationale below cap when system dialog grants then returns true`() = runTest {
            ...
            grantAll()
            (manager.pendingPermissionRequest.value as PermissionRequest.Improv)
                .onResult(improvPermissions.associateWith { true })
            advanceUntilIdle()
        ...
        fun `Given system dialog returns partial grant when called then returns false`() = runTest {
            ...
            (manager.pendingPermissionRequest.value as PermissionRequest.Improv).onResult(
                mapOf(android.Manifest.permission.BLUETOOTH_SCAN to true),
            )
            advanceUntilIdle()
